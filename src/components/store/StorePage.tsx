...
const StorePage = () => {
  ...
  const [storeOwner, setStoreOwner] = useState<Profile | null>(null);
  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      ...
      try {
        ...
        setStoreOwner(profile as Profile);

        // Fetch products for this store
        await fetchProducts((profile as Profile).id);
        ...
        if (categoriesData) {
          setCategories((categoriesData as any[]).map((cat: any) => cat.name));
        }
      } catch (error) {
